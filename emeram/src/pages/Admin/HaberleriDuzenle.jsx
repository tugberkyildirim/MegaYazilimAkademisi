...
export default function HaberleriDuzenle({ user }) {
    ...
    const handleAdd = async () => {
        try {
            if (baslik.trim().length > 0 && yazi.trim().length > 0) {
                ...
                await toast.success('Haber başarıyla eklendi.')
            } else {
                await toast.error('Lütfen Boş Alan Bırakmayınız')
            }
        } catch (error) {
            await toast.error('Haber eklenirken bir hata oluştu.')
        }
    }
    ...
    const handleUpdate = async () => {
        try {
            if (baslik.trim().length > 0 && yazi.trim().length > 0) {
                ...
                const contactRef = doc(db, 'haberler', selectedDocId)
                await updateDoc(contactRef, data)
                await toast.success('Haber Başarıyla Güncellendi')
            } else {
                await toast.error('Lütfen Boş Alan Bırakmayınız')
            }
        } catch (error) {
            await toast.error('Haber Güncellenirken Bir Hata Oluştu')
        }
    }
    ...
    const handleDelete = async () => {
        try {
            ...
            await toast.success('Haber Başarıyla Silindi')
            window.location.reload()
        } catch (error) {
            await toast.error('Haber Silinirken Bir Hata Oluştu')
        }
    }

    const handleEditClick = (docId) => {
        setSelectedDocId(docId)
        const docRef = doc(db, 'haberler', docId)
    ...
    return (<>
                ...
                <div className="card-body">
                    <div className="row  align-items-center">
                        <div className="col-lg-8 w-75">
                            <h2 className="m-0">Haber Listesi</h2>
                        </div>
            ...
            <div className="modal-dialog">
                <div className="modal-content">
                    <div className="modal-header">
                        <h5 className="modal-title" id="addModalLabel">Haber Ekle</h5>
        ...
        <div className="modal fade" id="editModal" tabIndex="-1" aria-labelledby="editModalLabel" aria-hidden="true">
            <div className="modal-dialog">
                <div className="modal-content">
                    <div className="modal-header">
                        <h5 className="modal-title" id="editModalLabel">Haber Düzenle</h5>
                        <button type="button" className="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                    </div>
                    <div className="modal-body">
                        <div className='p-3'>
        ...
        <div className="modal fade" id="deleteModal" tabIndex="-1" aria-labelledby="deleteModalLabel" aria-hidden="true">
            <div className="modal-dialog">
                <div className="modal-content">
                    <div className="modal-header">
                        <h5 className="modal-title" id="deleteModalLabel">Haber Sil</h5>
                        <button type="button" className="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                    </div>
                    <div className="modal-body">
                        <p>Emin misiniz?</p>
