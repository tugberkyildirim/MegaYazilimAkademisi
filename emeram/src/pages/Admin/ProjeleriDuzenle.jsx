...
export default function ProjeleriDuzenle({ user }) {
    ...
    const handleAdd = async () => {
        try {
            if (baslik.trim().length > 0 && durum.trim().length > 0 && yazi.trim().length > 0) {
                await addDoc(collection(db, 'projeler'), {
                    baslik: baslik,
                    durum: durum,
                    yazi: yazi,
                })
                await toast.success('Proje başarıyla eklendi.')
            } else {
                await toast.error('Lütfen Boş Alan Bırakmayınız')
            }
        } catch (error) {
            await toast.error('Proje eklenirken bir hata oluştu.')
        }
    }
    const handleSelectChange = (e) => {setDurum(e.target.value);};
    const handleUpdate = async () => {
        try {
            if (baslik.trim().length > 0 && durum.trim().length > 0 && yazi.trim().length > 0) {
                ...
                const contactRef = doc(db, 'projeler', selectedDocId)
                await updateDoc(contactRef, data)
                await toast.success('Proje Başarıyla Güncellendi')
            } else {
                await toast.error('Lütfen Boş Alan Bırakmayınız')
            }
        } catch (error) {
            await toast.error('Proje Güncellenirken Bir Hata Oluştu')
        }
    }
    ...
    const handleDelete = async () => {
        try {
            ...
            await toast.success('Proje Başarıyla Silindi')
            window.location.reload()
        } catch (error) {
            await toast.error('Proje Silinirken Bir Hata Oluştu')
        }
    }

    const handleEditClick = (docId) => {
        setSelectedDocId(docId);
        const docRef = doc(db, 'projeler', docId);
    ...
    return (<>
                ...
                <div className="card-body">
                    <div className="row  align-items-center">
                        <div className="col-lg-8 w-75">
                            <h2 className="m-0">Proje Listesi</h2>
                        </div>
                    ...
                    <div className={`collapse mt-3 ${isCollapsed ? 'show' : ''}`} id="projectList">
                                ...
                                <li key={item.docId} className="list-group-item d-flex justify-content-between align-items-center">
                                    <div className="row w-100">
                                        ...
                                        <div className='col-lg-2 float-end'>
                                            <h5>Durum</h5>
                                            <p className="text-truncate">{(item.durum == "fin") ? "Tamamlandı" : "Devam Ediyor"}</p>
                                        </div>
                                        <div className='col'>
                                            <button
                                                type="button"
            ...
            <div className="modal-dialog">
                <div className="modal-content">
                    <div className="modal-header">
                        <h5 className="modal-title" id="addModalLabel">Proje Ekle</h5>
                    ...
                    <div className="modal-body">
                        <div className='p-3'>
                            <div className="row mb-0 w-100">
                                ...
                                <div className="col-md-6 mt-3 w-100 px-0">
                                    <label className="form-label">Durum</label>
                                    <select name="" id="selectDurum" className="form-control" onChange={handleSelectChange}  required>
                                        <option value={"dvm"}>Devam Ediyor</option>
                                        <option value={"fin"}>Tamamlandı</option>
                                    </select>
                                </div>
                            </div>
                        </div>
        ...
        <div className="modal fade" id="editModal" tabIndex="-1" aria-labelledby="editModalLabel" aria-hidden="true">
            <div className="modal-dialog">
                <div className="modal-content">
                    <div className="modal-header">
                        <h5 className="modal-title" id="editModalLabel">Proje Düzenle</h5>
                        <button type="button" className="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                    </div>
                    <div className="modal-body">
                        <div className='p-3'>
                            <div className="row mb-0 w-100">
                                ...
                                <div className="col-md-6 mt-3 w-100 px-0">
                                    <label className="form-label">Durum</label>
                                    <select name="" id="selectDurum" className="form-control" onChange={handleSelectChange} value={durum}>
                                        <option value={"dvm"}>Devam Ediyor</option>
                                        <option value={"fin"}>Tamamlandı</option>
                                    </select>
                                </div>
        ...
        <div className="modal fade" id="deleteModal" tabIndex="-1" aria-labelledby="deleteModalLabel" aria-hidden="true">
            <div className="modal-dialog">
                <div className="modal-content">
                    <div className="modal-header">
                        <h5 className="modal-title" id="deleteModalLabel">Proje Sil</h5>
                        <button type="button" className="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                    </div>
                    <div className="modal-body">
                        <p>Emin misiniz?</p>
